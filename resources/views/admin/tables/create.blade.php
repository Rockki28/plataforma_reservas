...
<div class="card shadow-sm">
    <div class="card-header">
        <h5 class="mb-0">Formulario de Nueva Mesa</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.tables.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="capacity" id="capacity" class="form-control @error('capacity') is-invalid @enderror" value="{{ old('capacity', 2) }}" min="1" required>
                @error('capacity')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="location_description" id="location_description" class="form-control @error('location_description') is-invalid @enderror" value="{{ old('location_description') }}">
            ...
            <div class="mb-3">
                ...
                <select name="is_available" id="is_available" class="form-select @error('is_available') is-invalid @enderror" required>
                    <option value="1" {{ old('is_available', '1') == '1' ? 'selected' : '' }}>Sí</option>
            ...
            {{-- Si las mesas pertenecen a un Restaurante (Modelo Restaurant existe y se pasa $restaurants) --}}
            {{-- 
            ...
            <div class="mb-3">
                ...
                <select name="restaurant_id" id="restaurant_id" class="form-select @error('restaurant_id') is-invalid @enderror" required>
                    <option value="" disabled {{ old('restaurant_id') ? '' : 'selected' }}>Seleccione un restaurante</option>
            ...
            @else
                {{-- Asumir un solo restaurante o manejarlo de otra forma si no hay selección --}}
                {{-- <input type="hidden" name="restaurant_id" value="{{-- ID_RESTAURANTE_POR_DEFECTO -- }}"> --}}
            {{-- @endif 
            --}}
